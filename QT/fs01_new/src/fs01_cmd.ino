	...
	switch(udp_rx.cmd){
		case voice_download:
			if(udp_rx.seg_type==seg_save){
                len=udp_rx.seg_len;
                if(len > ima_seg_len)   len = ima_seg_len;
                memcpy(&ima_buf[voice_len],udp_rx.data,len);
                voice_len += len;
				if(write_buf("/a_"+String(udp_rx.page_id & 0xf)+".wav",ima_buf,voice_len))	led_reset=10;
			}else if(voice_len < (ima_seg_max-1)*ima_seg_len){
				if(udp_rx.seg_type==seg_begin)	voice_len=0;
				len=udp_rx.seg_len;
				if(len > ima_seg_len)	len = ima_seg_len;
				memcpy(&ima_buf[voice_len],udp_rx.data,len);
                voice_len += len;
		...
		case voice_play:
            ...
            file_len=voice_len;
            memcpy(file_buf,ima_buf,file_len);
            file_seg=0;
			break;
		case file_rename:
			break;
